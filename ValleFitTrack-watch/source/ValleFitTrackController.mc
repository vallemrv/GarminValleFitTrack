...
class ValleFitTrackController {
  ...
  var segundos = 0;         //segundos de la actividad en total
    ...
    function onNextLap(){
        if (set > 0 && (status == STARTED_REST || rest_elapsed >= TIME_REST)){
             mSession.addLap();
    ...
    function openModPesoDg(){
        WatchUi.pushView(new NumberPicker("Rep", 100),
                         new NumberPickerDelegate("Peso", "Rep", 250, repeticiones_serie, peso_serie),
    ...
    function runVibrate(s){
      if (Attention has :vibrate) {
            var vibrateData = [ new Attention.VibeProfile( 100, s ),];
